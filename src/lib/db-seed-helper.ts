...
export async function seedDemoUserData(userId: string = "demo-user-id") {
  try {
    // 1. Ensure user exists
    const hashedPassword = await bcrypt.hash("demo1234", 12);
    
    // First, delete any existing user with this email but a different ID to avoid unique constraint conflict
    ...
    // 3. Seed sample activities for the last 30 days
    const categories = [
